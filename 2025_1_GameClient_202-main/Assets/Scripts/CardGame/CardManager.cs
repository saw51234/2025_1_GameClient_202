...
public class CardManager : MonoBehaviour
{
    ...
    public void DrawCard()
    {
        ...
        ToastMessage.Instance.ShowMessage("카드를 드로우 했습니다. \n" + cardData.cardName + " ( 손패 : " + handCards.Count + "/6)", ToastMessage.MessageType.info);
    }
    ...
    public void ArrangeHand()
    {
        ...
        for(int i = 0; i < cardObjects.Count; i++)
        {
            if(cardObjects[i] != null)
            {
                ...
                Vector3 tarPos = handPosition.position + new Vector3(startX + (i * spacing), 0, 0);

                cardObjects[i].transform.position = Vector3.Lerp(cardObjects[i].transform.position, tarPos, Time.deltaTime * 10f);
            }
        }
    }
